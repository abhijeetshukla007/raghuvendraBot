...
class Zomato:
    ...
    def restaurant_search_price_range(self, query="", latitude="", longitude="", cuisines="", mincft="" , maxcft="",limit=5):
        """
        Takes either query, latitude and longitude or cuisine as input.
        Returns a list of Restaurant IDs.
        """
        ...
        if str(limit).isalpha() == True:
            raise ValueError('LimitNotInteger')
        headers = {'Accept': 'application/json', 'user-key': self.user_key}
        r = (requests.get(base_url + "search?q=" + str(query) + "&count=" + str(limit) + "&lat=" + str(latitude) + "&lon=" + str(longitude) + "&cuisines=" + str(cuisines) +"&mincft=" + str(mincft) + "&maxcft=" + str(maxcft) + "&sort=" + str("rating"), headers=headers).content).decode("utf-8")
        return r#a = ast.literal_eval(r)
